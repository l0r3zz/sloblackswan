...
def generate_epub(md_path, output_epub_path, output_dir, title=None, author=None):
...
'''
    ...
    sections = re.split(r'^(# .+)$', html_ready_md, flags=re.MULTILINE)
    
    chapters = []
    chapter_num = 0
    
    if sections[0].strip():
        intro_html = markdown.markdown(sections[0], extensions=['extra', 'codehilite', 'tables'])
        intro_html = update_image_paths(intro_html, image_files)
        intro = epub.EpubHtml(
            title='Introduction',
            file_name='intro.xhtml',
            lang='en'
        )
        intro.content = f'<html><head><link rel="stylesheet" href="style/nav.css"/></head><body>{intro_html}</body></html>'
    ...
    for i in range(1, len(sections), 2):
        if i + 1 < len(sections):
            ...
            chapter_title = heading.replace('# ', '').strip()
            
            full_content = heading + '\n' + content
            chapter_html = markdown.markdown(full_content, extensions=['extra', 'codehilite', 'tables'])
            chapter_html = update_image_paths(chapter_html, image_files)
            ...
            chapter.content = f'<html><head><link rel="stylesheet" href="style/nav.css"/></head><body>{chapter_html}</body></html>'
    ...
    if not chapters:
        ...
        full_html = update_image_paths(full_html, image_files)
        single_chapter = epub.EpubHtml(
            title=title,
            file_name='content.xhtml',
            lang='en'
        )
        single_chapter.content = f'<html><head><link rel="stylesheet" href="style/nav.css"/></head><body>{full_html}</body></html>'
...
def update_image_paths(html_content, image_files):
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python generate_epub.py <md_path> <output_epub_path> <output_dir> [title] [author]")
        sys.exit(1)
    ...
    generate_epub(md_path, output_epub_path, output_dir, title, author)
